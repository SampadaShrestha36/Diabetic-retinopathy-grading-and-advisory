"""
Step 6 (EfficientNet-B0 variant): Training loop.
Two-stage fine-tuning + weighted loss (for class imbalance) + QWK tracking.

Identical pipeline to train.py (ResNet-50), swapped to an EfficientNet-B0
backbone via model_efficientnet.py. Checkpoints/best-model files are saved
under *_efficientnet names so they don't collide with your other runs.
"""
...
from model_efficientnet import build_model, freeze_backbone, unfreeze_backbone
...
if __name__ == "__main__":
    ...
    # Reuses the SAME split files as the ResNet-50/DenseNet-121 runs (same
    # random_state=42 and same source CSV), so all backbones are trained/
    # evaluated on identical train/val partitions for a fair comparison.
    ...
    model, _ = train_one_stage(model, train_loader, val_loader, optimizer_stage1,
                                criterion, num_epochs=5, stage_name="stage1_efficientnet")

    # ---- Stage 2: unfreeze deeper layers, lower LR across all trainable params ----
    # features.6 onward is EfficientNet-B0's equivalent of ResNet-50/
    # DenseNet-121's "roughly back half of the backbone" default -
    # features.0-5 stay frozen as generic low-level features.
    ...
    model, best_qwk = train_one_stage(model, train_loader, val_loader, optimizer_stage2,
                                       criterion, num_epochs=60, stage_name="stage2_efficientnet", patience=8)
